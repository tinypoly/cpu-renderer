...
/** The two calls a worker host needs from its transport: send a response, and end the thread on `dispose`. */
...
/**
 * The render worker, independent of the transport: `worker.ts` binds it to a Web Worker's global scope and
 * `workerNode.ts` to a `worker_threads` port. Returns the handler for incoming requests.
 *
 ...
 */
export function createWorkerHost(transport: WorkerTransport): (request: WorkerRequest) => void {
  let revision = 0;
  ...
  function withoutComputeSnapshots(source: SerializedScene): SerializedScene {
    ...
    const meshes = source.meshes.map(mesh => ({ ...mesh, materials: mesh.materials.map(material => {
      let copy = copies.get(material);

      if (!copy) {
        copy = material.compute?.snapshot
          ? { ...material, compute: { ...material.compute, snapshot: null } } : material;
